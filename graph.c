               ...
               const char *name)
{
    ...
    strncpy(graph->stations[id].stationName,
            name,
            NAME_LEN - 1U);
 
    graph->stations[id].stationName[
        NAME_LEN - 1U] = '\0';
...
void removeStation(Graph *graph,
                   int stationId)
{
    if(graph == NULL)
    {
                ...
                const char *name)
{
   ...
   (strcmp(graph->stations[i].stationName,
           name) == 0))
        {
            return (int)i;
        }
